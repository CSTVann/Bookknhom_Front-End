...
export default async function handler(req, res){
    if (req.method === 'POST') {
        ...
        try {
          const response = await axios.post(`${configs.host}/api/recorddata`, formDatas);
        ...
        try {
          const response = await axios.get(`${configs.host}/api/recorddata`);
          res.status(200).json(response.data);
        } catch (error) {
          console.error('Error fetching data from ${configs.host}/api/recorddata:', error);
          res.status(500).json({ error: 'Internal Server Error' });
        }
      } 
}
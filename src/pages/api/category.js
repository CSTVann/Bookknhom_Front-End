...
export default async function handler(req, res) {
  ...
  if (method === 'DELETE') {
    ...
    try {
      await axios.delete(`${configs.host}/api/category/${categoryID}`);
    ...
    try {
      const response = await axios.post(`${configs.host}/api/category`, formData);
    ...
    try {
      await axios.put(`${configs.host}/api/category/${categoryID}`, formData);
    ...
    try {
      ...
      console.error('Error fetching data from ${configs.host}/api/category', error);
      res.status(500).json({ error: 'Internal Server Error' });
    }
  }
}

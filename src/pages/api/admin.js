...
export default async function handler(req, res) {
  if (req.method === 'DELETE') {
    ...
    try {
      await axios.delete(`${configs.host}/api/books/destroy/${adminID}`);
    ...
    try {
      await axios.put(`${configs.host}/api/books/update/${adminID}`, formData);
    ...
    try {
      ...
      console.error('Error fetching data from ${configs.host}/api/books/index:', error);
      res.status(500).json({ error: 'Internal Server Error' });
    }
  }
}

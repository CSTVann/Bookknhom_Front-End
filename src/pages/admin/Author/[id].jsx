...
const EditBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${configs.host}/api/authors/${id}`, data);
  ...
  useEffect(() => {
    if (router.query.author) {
      const parseData = JSON.parse(router.query.author)
      ...
      <form onSubmit={handleSubmit} className=''>
        <div className='container-back-author-edit'>
          <Link href="/admin/author">
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="20" viewBox="0 0 11 20" fill="none">
          ...
          <div className='container-yyybbbyyy'>
            <label>Name:   </label>
            <span type="number" name="name" onChange={handleChange} >{authorData.name}</span>
          </div>
        </div>
        <div className='container-form-edit-author container-author-post'>
          <h2 className=''>Page for Admin to edit everything</h2>
          <div className='form-author-edit'>
            
            <div className=''>
              <div className='author-edit-button container-author-form'>
                <label>Name:</label>
                <input type="text" name="name" value={authorData.name} onChange={handleChange} />
              </div>
              <div className='author-edit-button container-author-form'>
